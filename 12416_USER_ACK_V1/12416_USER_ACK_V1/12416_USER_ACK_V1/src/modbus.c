
/**
 * Implements utility functions for MODBUS communication over UART
 *
 * Interfaces to the UART via a circular buffer.  Reads incoming characters
 * and stores them in a flat buffer until the buffer either overflows or
 * sufficient time has elapsed with no more incoming data (a MODBUS RTU end
 * of frame).
 *
 * Also provides functions to send MODBUS packets over the UART, calculating
 * the RTU checksum and appending it.
 *
 */
#include "modbus.h"
#include "app.h"
//#include "timer.h"
#include "system.h"
#include "uart.h"
#include <stdint.h>

//------------------------------//----------------------------------------------
// Definitions

//------------------------------//----------------------------------------------
// Global variables

// buffer that collects the incoming message
uint8_t MODBUS_rxBuffer[ MODBUS_MAX_ADU ];
uint8_t MODBUS_rxBufferIndex;

//------------------------------//----------------------------------------------
// Local variables

// CRC accumulators
static unsigned char _ucCRCHi;
static unsigned char _ucCRCLo;

// rx serial line must be idle at least this long to declare end of frame

static TIMER_TICK _lastRxCharacterTime;

// helper tables for computing CRCs
static const uint8_t aucCRCHi[] = {
    };

static const uint8_t aucCRCLo[] = {
    };

//------------------------------//----------------------------------------------
// Local functions

/**
 * Calculates a MODBUS CRC over a buffer
 *
 */
static uint16_t MODBUS_CRC( uint8_t *pFrame, uint16_t usLen )
{
    uint8_t ucCRCHi = 0xFF;
    uint8_t ucCRCLo = 0xFF;

    while( usLen-- )
    {
        int iIndex = ucCRCLo ^ *( pFrame++ );
        ucCRCLo = ( uint8_t )( ucCRCHi ^ aucCRCHi[ iIndex ] );
        ucCRCHi = aucCRCLo[ iIndex ];
    }

    return ( uint8_t )( ( ucCRCHi << 8 ) | ucCRCLo );
}

/**
 * Update checksum accumulators with outgoing byte and send it to the port
 */
static void _Send( uint8_t c )
{
    int iIndex;

    // Update the CRC accumulators
    iIndex = _ucCRCLo ^ c;
    _ucCRCLo = (unsigned char)( _ucCRCHi ^ aucCRCHi[ iIndex ] );
    _ucCRCHi = aucCRCLo[ iIndex ];

    // Write out byte to serial port buffer
    UART1_Put( c );
}

/**
 * Clear the checksum and write the message header to the port
 */
static void _StartSend()
{
    // reset the CRC accumulators
    _ucCRCHi = 0xFF;
    _ucCRCLo = 0xFF;
}

/**
 * Write the end of message (checksum) to the port
 */
static void _FinishSend()
{
    UART1_Put( _ucCRCLo );
    UART1_Put( _ucCRCHi );
}

//------------------------------//----------------------------------------------
// Public functions

/**
 * Powerup initialization
 */
void MODBUS_Initialize( void )
{
    // Don't open the port here, wait until MODBUS_Open())
}

/**
 * Maintenance tasks loop
 */
void MODBUS_Tasks( void )
{
    // nothing to do here
}

/**
 * Opens the UART port using the specified baud rate
 */
bool MODBUS_Open( UartBaud_e baud )
{
    MODBUS_DiscardMessage();
    
    return UART1_Initialize();//UART_Open( baud );
}

/**
 * Close the serial port
 */
void MODBUS_Close( void )
{
    //UART_Close();
}

/**
 * Sends a MODBUS ADU
 *
 * Loads the ADU into the outgoing UART buffer.  Does not check
 * for bus idle conditions, etc - simply sends the bytes.  The
 * checksum is calculated and appended to the message
 */
bool MODBUS_Send( uint8_t slaveId, uint8_t function, uint8_t *pData, uint8_t length )
{
    _StartSend();
    _Send( slaveId );
    _Send( function );

    while( length > 0 )
    {
        _Send( *pData++ );
        length--;
    }
    _FinishSend();

    return true;
}

/**
 * Sends a MODBUS ADU from a buffer
 *
 * Loads the ADU into the outgoing UART buffer.  The buffer
 * should contain the whole message EXCEPT the crc, which will
 * be added by this function
 */
bool MODBUS_SendBuffer( uint8_t *pData, uint8_t length )
{
    _StartSend();

    while( length > 0 )
    {
        _Send( *pData++ );
        length--;
    }
    _FinishSend();

    return true;
}

/**
 * Flushes the receive PDU buffer
 *
 * Note: any buffers at the UART level are not flushed; simply the
 * accumulated PDU
 */
void MODBUS_DiscardMessage( void )
{
    MODBUS_rxBufferIndex = 0;
}

/**
 * Interface to check if there is a complete incoming MODBUS message
 *
 * This function reads from the UART, accumulates the ADU, and validates
 * the checksum and end of frame.
 * If it returns true, it means there is a complete MODBUS message in the
 * UART_rxBuffer
 */
bool MODBUS_MessageAvailable( void )
{
    // For non-interrupt driven UART interfaces
    //UART_Poll();

    // read in any incoming bytes
    while( UART1_IsGetReady() )
    {
        MODBUS_rxBuffer[ MODBUS_rxBufferIndex++ ] = UART1_Get();
        if( MODBUS_rxBufferIndex >= MODBUS_MAX_ADU )
        {
            MODBUS_rxBufferIndex = 0;
        }

        // keep track of the time mark
        _lastRxCharacterTime = TIMER_Now();
    }
    
    
    
    if( ( MODBUS_rxBufferIndex > 0 ) && ( TIMER_Since( _lastRxCharacterTime ) > MODBUS_EOF_MS ) )
    {
        bool bValid = false;
        //printf("%d", MODBUS_rxBufferIndex);
        // minimum incoming is ID, Function, CRCH and CRCL
        if( MODBUS_rxBufferIndex >= 4 )
        {
            // calculate CRC over entire buffer
            uint16_t crc = MODBUS_CRC( MODBUS_rxBuffer, (uint16_t)MODBUS_rxBufferIndex );

            bValid = ( crc == 0 );
        }

        if( !bValid )
        {
            MODBUS_DiscardMessage();
        }

        return bValid;
    }

    return false;
}
