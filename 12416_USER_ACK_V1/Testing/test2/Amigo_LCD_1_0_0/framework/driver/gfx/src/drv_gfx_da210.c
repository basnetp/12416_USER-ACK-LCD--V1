...
#ifndef GFX_CONFIG_PALETTE_DISABLE
static GFX_STATUS DRV_GFX_ColorLookUpTableSet( const GFX_PALETTE_ENTRY *pPaletteEntry, uint16_t start, uint16_t count );
#endif
...
*/
// *****************************************************************************
static inline uint16_t __attribute__( ( always_inline ) ) GFX_FreeCommandSpaceCheck( uint16_t spaceNeeded )
{
...
void DRV_GFX_Initialize( void )
{
    ...
    //    DisplayPowerOn();//PIC24FJ256DA210
    //    DisplayPowerConfig();//PIC24FJ256DA210
...
*/
// *****************************************************************************
inline uint16_t __attribute__( ( always_inline ) )
ROPBlockInline( uint32_t srcAddr, uint32_t dstAddr, uint32_t srcOffset, uint32_t dstOffset, uint16_t srcType,
                uint16_t dstType, uint16_t copyOp, uint16_t rop, uint16_t color, uint16_t width, uint16_t height )
{
...
*/
// *****************************************************************************
uint16_t DRV_GFX_BitBlt( uint32_t srcAddr, uint32_t dstAddr, uint32_t srcOffset, uint32_t dstOffset, uint16_t srcType,
                         uint16_t dstType, uint16_t copyOp, uint16_t rop, uint16_t color, uint16_t width,
                         uint16_t height )
{
    return (
        ROPBlockInline( srcAddr, dstAddr, srcOffset, dstOffset, srcType, dstType, copyOp, rop, color, width, height ) );
}
...
*/
// *****************************************************************************
uint16_t DRV_GFX_BlockOffsetCopy( uint32_t srcAddr, uint32_t dstAddr, uint32_t srcOffset, uint32_t dstOffset,
                                  uint16_t width, uint16_t height )
{

    return ( ROPBlockInline( srcAddr, dstAddr, srcOffset, dstOffset, ( GFX_DATA_DISCONTINUOUS ),
                             ( GFX_DATA_DISCONTINUOUS ), RCC_COPY, RCC_ROP_C, 0, width, height ) );
}
...
*/
// *****************************************************************************
uint16_t DRV_GFX_BlockCopy( uint32_t srcAddr, uint32_t dstAddr, uint16_t srcX, uint16_t srcY, uint16_t dstX,
                            uint16_t dstY, uint16_t width, uint16_t height )
{
...
// are calculated.
    ...
    DRV_GFX_BitBlt( srcAddr, dstAddr, srcOffset, dstOffset, GFX_DATA_DISCONTINUOUS, GFX_DATA_DISCONTINUOUS, RCC_COPY,
                    RCC_ROP_C, 0, actualWidth, actualHeight );
...
uint16_t GFX_PixelArrayGet( uint16_t x, uint16_t y, GFX_COLOR *pPixel, uint16_t numPixels )
{
    ...
    GFX_RCC_SetSize( width, height );
    GFX_RCC_StartCopy( RCC_COPY, RCC_ROP_C, RCC_SRC_ADDR_DISCONTINUOUS, RCC_DEST_ADDR_CONTINUOUS );
...
*/
// *****************************************************************************
GFX_STATUS GFX_StyledLineDraw( uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2 )
{
    /*
...
*/
// *****************************************************************************
uint16_t GFX_DRV_TextStringWidthGet( GFX_XCHAR *pString, GFX_RESOURCE_HDR *pFont )
{
    ...
    while( (GFX_XCHAR)15 < ( GFX_XCHAR )( ch = *pStr++ ) )
    {
        ...
        offset = sizeof( GFX_FONT_HEADER ) +
                 ( ( (GFX_XCHAR)ch - (GFX_XCHAR)fontFirstChar ) * sizeof( GFX_FONT_GLYPH_ENTRY ) );
        while( !DRV_GFX_BitBlt( edsFontAddr, ( uint32_t )(uint16_t)&temp, offset, 0, GFX_DATA_CONTINUOUS,
                                GFX_DATA_CONTINUOUS, RCC_COPY, RCC_ROP_C, 0, 1, 1 ) )
            ;
...
GFX_STATUS GFX_DRV_TextCharRender( GFX_XCHAR ch )
{
    ...
    GFX_CHR_SetPrintPos( GFX_TextCursorPositionXGet(), GFX_TextCursorPositionYGet() );
...
*/
// *****************************************************************************
void GFX_ImageFlash1BPPDraw( uint16_t left, uint16_t top, GFX_RESOURCE_HDR *pImage,
                             GFX_PARTIAL_IMAGE_PARAM *pPartialImageData )
{
    ...
    if( pPartialImageData->width != 0 )
    {
        ...
        flashAddress +=
            ( ( uint16_gfx_image_prog )( pPartialImageData->yoffset ) * ( ( bitmapHdr.width >> 3 ) + adjOffset ) );
...
#elif( DISP_ORIENTATION == 180 )
        GFX_RCC_SetDestOffset( ( ( GFX_MaxYGet() - yc ) * (uint32_t)DISP_HOR_RESOLUTION ) + GFX_MaxXGet() + 1 -
                               ( left + outputSize ) );
#elif( DISP_ORIENTATION == 270 )
...
*/
// *****************************************************************************
void GFX_ImageFlash4BPPDraw( uint16_t left, uint16_t top, GFX_RESOURCE_HDR *pImage,
                             GFX_PARTIAL_IMAGE_PARAM *pPartialImageData )
{
    register uint8_gfx_image_prog *flashAddress;
    register uint8_gfx_image_prog *tempFlashAddress;
    ...
    if( pPartialImageData->width != 0 )
    {
        ...
        // adjust one address if the data is not byte aligned
        flashAddress +=
            ( ( uint16_gfx_image_prog )( pPartialImageData->yoffset ) * ( ( bitmapHdr.width >> 1 ) + adjOffset ) );
    ...
    for( y = 0; ( y < bitmapHdr.height ) && ( yc <= GFX_MaxYGet() ); y++ )
    {
        ...
        GFX_RCC_SetDestOffset( ( yc * (uint32_t)DISP_HOR_RESOLUTION ) + left );
#elif( DISP_ORIENTATION == 90 )
        GFX_RCC_SetDestOffset(
            ( ( GFX_MaxXGet() + 1 - ( left + outputSize - nibbleOffset ) ) * (uint32_t)DISP_HOR_RESOLUTION ) + yc );
#elif( DISP_ORIENTATION == 180 )
        GFX_RCC_SetDestOffset( ( ( GFX_MaxYGet() - yc ) * (uint32_t)DISP_HOR_RESOLUTION ) + GFX_MaxXGet() + 1 -
                               ( left + outputSize - nibbleOffset ) );
#elif( DISP_ORIENTATION == 270 )
        GFX_RCC_SetDestOffset( ( left * (uint32_t)DISP_HOR_RESOLUTION ) + ( GFX_MaxYGet() - yc ) );
#endif
...
*/
// *****************************************************************************
void GFX_ImageFlash8BPPDraw( uint16_t left, uint16_t top, GFX_RESOURCE_HDR *pImage,
                             GFX_PARTIAL_IMAGE_PARAM *pPartialImageData )
{
    register uint8_gfx_image_prog *flashAddress;
    register uint8_gfx_image_prog *tempFlashAddress;
...
*/
// *****************************************************************************
void GFX_ImageExternal1BPPDraw( uint16_t left, uint16_t top, GFX_RESOURCE_HDR *pImage,
                                GFX_PARTIAL_IMAGE_PARAM *pPartialImageData )
{
...
*/
// *****************************************************************************
void GFX_ImageExternal4BPPDraw( uint16_t left, uint16_t top, GFX_RESOURCE_HDR *pImage,
                                GFX_PARTIAL_IMAGE_PARAM *pPartialImageData )
{
...
*/
// *****************************************************************************
void __attribute__( ( weak ) ) GFX_ImageExternal8BPPDraw( uint16_t left, uint16_t top, GFX_RESOURCE_HDR *pImage,
                                                          GFX_PARTIAL_IMAGE_PARAM *pPartialImageData )
{
    register uint32_t memOffset;
    GFX_MCHP_BITMAP_HEADER bitmapHdr;
...
*/
// *****************************************************************************
void __attribute__( ( weak ) ) GFX_ImageEDS1BPPDraw( uint16_t left, uint16_t top, GFX_RESOURCE_HDR *pImage,
                                                     GFX_PARTIAL_IMAGE_PARAM *pPartialImageData )
{
    uint8_t TempGPUBpp;
    uint8_t lineBuffer[ ( ( GFX_MaxXGet() + (uint32_t)1 ) / 8 ) + 1 ];
...
*/
// *****************************************************************************
void __attribute__( ( weak ) ) GFX_ImageEDS4BPPDraw( uint16_t left, uint16_t top, GFX_RESOURCE_HDR *pImage,
                                                     GFX_PARTIAL_IMAGE_PARAM *pPartialImageData )
{
    uint8_t TempGPUBpp;
...
*/
// *****************************************************************************
void __attribute__( ( weak ) ) GFX_ImageEDS8BPPDraw( uint16_t left, uint16_t top, GFX_RESOURCE_HDR *pImage,
                                                     GFX_PARTIAL_IMAGE_PARAM *pPartialImageData )
{
    uint8_t TempGPUBpp;
    uint8_t lineBuffer[ ( GFX_MaxXGet() + (uint32_t)1 ) ];
...
*/
// *****************************************************************************
void __attribute__( ( weak ) ) GFX_ImageEDS16BPPDraw( uint16_t left, uint16_t top, GFX_RESOURCE_HDR *pImage,
                                                      GFX_PARTIAL_IMAGE_PARAM *pPartialImageData )
{
    ...
    if( pPartialImageData->width == 0 )
    {
        ...
        DRV_GFX_BitBlt( ( (GFX_RESOURCE_HDR *)pImage )->resource.image.location.extAddress, drvGfxDrawBufferBaseAddress,
                        0, ( top * (uint32_t)DISP_HOR_RESOLUTION ) + left, GFX_DATA_CONTINUOUS, GFX_DATA_DISCONTINUOUS,
                        RCC_TRANSPARENT_COPY, RCC_ROP_C, GFX_TransparentColorGet(), bitmapHdr.width, bitmapHdr.height );
#else
        DRV_GFX_BitBlt( ( (GFX_RESOURCE_HDR *)pImage )->resource.image.location.extAddress,
                        drvGfxFrameBufferBaseAddress, 0, ( top * (uint32_t)DISP_HOR_RESOLUTION ) + left,
                        GFX_DATA_CONTINUOUS, GFX_DATA_DISCONTINUOUS, RCC_TRANSPARENT_COPY, RCC_ROP_C,
                        GFX_TransparentColorGet(), bitmapHdr.width, bitmapHdr.height );
#endif
        ...
        DRV_GFX_BitBlt( ( (GFX_RESOURCE_HDR *)pImage )->resource.image.location.extAddress, drvGfxDrawBufferBaseAddress,
                        0, ( top * (uint32_t)DISP_HOR_RESOLUTION ) + left, GFX_DATA_CONTINUOUS, GFX_DATA_DISCONTINUOUS,
                        RCC_COPY, RCC_ROP_C, 0, bitmapHdr.width, bitmapHdr.height );
#else

        DRV_GFX_BitBlt( ( (GFX_RESOURCE_HDR *)pImage )->resource.image.location.extAddress,
                        drvGfxFrameBufferBaseAddress, 0, ( top * (uint32_t)DISP_HOR_RESOLUTION ) + left,
                        GFX_DATA_CONTINUOUS, GFX_DATA_DISCONTINUOUS, RCC_COPY, RCC_ROP_C, 0, bitmapHdr.width,
                        bitmapHdr.height );
#endif
#endif
    }
...
#endif  // #if (DISP_ORIENTATION == 0)
    {
        ...
        for( y = 0; ( y < bitmapHdr.height ) && ( actualY <= GFX_MaxYGet() ); y++ )
        {
            ...
            if( GFX_TransparentColorStatusGet() == GFX_FEATURE_ENABLED )
            {
                GFX_RCC_StartCopy( RCC_TRANSPARENT_COPY, RCC_ROP_C, RCC_SRC_ADDR_CONTINUOUS,
                            ...
                            GFX_PARTIAL_IMAGE_PARAM *pPartialImageData )
{
    ...
    if( ( resType & GFX_COMP_MASK ) == GFX_RESOURCE_COMP_IPU )
    {
        ...
        if( ( ( resType & GFX_MEM_MASK ) == GFX_RESOURCE_MEMORY_EDS_EPMP ) ||
            ( ( resType & GFX_MEM_MASK ) == GFX_RESOURCE_MEMORY_RAM ) )
        {
            while( GFX_FreeCommandSpaceCheck( GFX_COMMAND_QUEUE_LENGTH ) == 0 )
                ;
            GFX_WaitForGpu();
        ...
        else if( ( resType & GFX_MEM_MASK ) == GFX_RESOURCE_MEMORY_FLASH )
        {
            ...
            remainder =
                piHdr->resource.image.parameter1.compressedSize - ( quotient * GFX_IPU_TEMP_DATA_TRANSFER_ARRAY_SIZE );
            ...
            while( quotient )
            {
                ...
                DRV_GFX_BitBlt( srcaddress, GFX_COMPRESSED_DATA_RAM_ADDRESS, 0, offset, GFX_DATA_CONTINUOUS,
                                GFX_DATA_CONTINUOUS, RCC_COPY, RCC_ROP_C, 0, GFX_IPU_TEMP_DATA_TRANSFER_ARRAY_SIZE, 1 );
            ...
            while( GFX_FreeCommandSpaceCheck( 4 ) == 0 )
                ;
            DRV_GFX_BitBlt( srcaddress, GFX_COMPRESSED_DATA_RAM_ADDRESS, 0, offset, GFX_DATA_CONTINUOUS,
                            GFX_DATA_CONTINUOUS, RCC_COPY, RCC_ROP_C, 0, remainder, 1 );
    ...
    {
        case 1:
            GFX_ImageEDS1BPPDraw( left, top, (GFX_RESOURCE_HDR *)image, pParImageData );
            break;

#if( GFX_CONFIG_COLOR_DEPTH >= 4 )
        case 4:
            GFX_ImageEDS4BPPDraw( left, top, (GFX_RESOURCE_HDR *)image, pParImageData );
            break;
#endif

#if( GFX_CONFIG_COLOR_DEPTH >= 8 )
        case 8:
            GFX_ImageEDS8BPPDraw( left, top, (GFX_RESOURCE_HDR *)image, pParImageData );
            break;
#endif

#if( GFX_CONFIG_COLOR_DEPTH == 16 )
        case 16:
            GFX_ImageEDS16BPPDraw( left, top, (GFX_RESOURCE_HDR *)image, pParImageData );
            break;
#endif
...
*/
// *****************************************************************************
uint8_t DRV_GFX_Inflate( uint32_t srcAddress, uint32_t destAddress, uint32_t nBytes )
{
    while( GFX_FreeCommandSpaceCheck( GFX_COMMAND_QUEUE_LENGTH ) == 0 )
        ;
...
GFX_STATUS GFX_DoubleBufferSynchronize( void )
{
        ...
        GFX_DoubleBufferSyncAreaCountGet() > GFX_MAX_INVALIDATE_AREAS )
    {
        ...
        GFX_RCC_SetSrcOffset( 0 );
        GFX_RCC_SetDestOffset( 0 );
        GFX_RCC_SetSize( (uint16_t)DISP_HOR_RESOLUTION, (uint16_t)DISP_VER_RESOLUTION );
        GFX_RCC_StartCopy( RCC_COPY, RCC_ROP_C, RCC_SRC_ADDR_CONTINUOUS, RCC_DEST_ADDR_CONTINUOUS );
...
GFX_STATUS DRV_GFX_PaletteColorDepthSet( uint8_t depth )
{
    ...
    {
        case 1:
            PaletteBpp = GFX_1_BPP;
            break;

        case 2:
            PaletteBpp = GFX_2_BPP;
            break;

        case 4:
            PaletteBpp = GFX_4_BPP;
            break;

        case 8:
            PaletteBpp = GFX_8_BPP;
            break;

        default:
            PaletteBpp = GFX_16_BPP;
            return GFX_STATUS_FAILURE;
    }
...
*/
// *****************************************************************************
GFX_STATUS DRV_GFX_PaletteSet( const GFX_PALETTE_ENTRY *pPaletteEntry, uint16_t start, uint16_t count )
{
...
*/
// *****************************************************************************
inline GFX_STATUS __attribute__( ( always_inline ) )
DRV_GFX_ColorLookUpTableSet( const GFX_PALETTE_ENTRY *pPaletteEntry, uint16_t start, uint16_t count )
{
...
*/
// *****************************************************************************
GFX_STATUS GFX_PaletteSet( GFX_RESOURCE_HDR *pPalette, uint16_t start, uint16_t count )

{
    ...
    switch( pPalette->type )
    {
        ...
        case GFX_RESOURCE_PALETTE_FLASH_NONE:
            ...
            status = DRV_GFX_PaletteSet( pPalette->resource.palette.location.constByteAddress, start, count );
        ...
        case GFX_RESOURCE_PALETTE_RAM_NONE:
            ...
            status = DRV_GFX_PaletteSet( pPalette->resource.palette.location.ramAddress, start, count );
        ...
        case GFX_RESOURCE_PALETTE_EXTERNAL_NONE:
            ...
            */
            GFX_ExternalResourceCallback( pPalette, ( startEntry * sizeof( GFX_PALETTE_ENTRY ) ),
                                          sizeof( GFX_PALETTE_ENTRY ) * length, (void *)paletteEntry );
...
void __attribute__( ( interrupt, shadow, no_auto_psv ) ) _GFX1Interrupt( void )
{
    ...
    if( _VMRGNIF )
    {
        ...
        {
            DRV_GFX_ColorLookUpTableSet( pPendingPaletteEntry, PendingStartEntry, PendingLength );
